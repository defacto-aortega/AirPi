...
class BMP280(sensor.Sensor):
    ...
    optionalOptions = ["unit", "address"]

    def __init__(self, data):

        self.address = 0x76
        self.i2cbus = int(data["i2cbus"])

        if "address" in data:
            self.address = int(data["address"], 0)

        self.sensorName = "BMP280"
    ...
    def getData(self):
        i2c = I2C
        device = i2c.get_i2c_device(address=self.address, busnum=self.i2cbus)
        ...
        if device.readS8(BMP280_REGISTER_CHIPID) == 0x58:  # check sensor id 0x58=BMP280
            ...
            raw_temp = (raw_temp_msb << 12) + (raw_temp_lsb << 4) + (
                raw_temp_xlsb >> 4)  # combine 3 bytes  msb 12 bits left, lsb 4 bits left, xlsb 4 bits right
            raw_press = (raw_press_msb << 12) + (raw_press_lsb << 4) + (
                raw_press_xlsb >> 4)  # combine 3 bytes  msb 12 bits left, lsb 4 bits left, xlsb 4 bits right
            ...
            var2 = (raw_temp / 131072.0 - dig_T1 / 8192.0) * (
                raw_temp / 131072.0 - dig_T1 / 8192.0) * dig_T3  # formula for temperature from datasheet
            ...
            altitude = 44330.0 * (
                1.0 - pow(press / (QNH * 100), (1.0 / 5.255)))  # formula for altitude from airpressure

            # print(
            #    "temperature:{:.2f}".format(temp) + " C  pressure:{:.2f}".format(
            #        press / 100) + " hPa   altitude:{:.2f}".format(
            #        altitude) + " m")
            return temp, press / 100, altitude
